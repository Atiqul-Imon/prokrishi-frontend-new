...
export default function EditProductPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          measurementIncrement: product.measurementIncrement?.toString() || "1",
        ...
        if (product.hasVariants && product.variants && product.variants.length > 0) {
          ...
          setVariants(
            product.variants.map((v: ProductVariantType) => ({
              ...
              measurementIncrement: v.measurementIncrement?.toString() || "1",
  ...
  const handleGalleryChange = (images: Array<{ file?: File; preview: string; url?: string }>) => {
    ...
    const existing = images.filter((img) => img.url && !img.file).map((img) => img.url!);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingGallery.length === 0 && galleryUploads.length === 0) {
        productData.append("removeGallery", "true");
      }
      ...
      const errorMessage = handleApiError(err, "updating product");
  ...
  const allGalleryImages = [
    ...existingGallery.map((url) => ({ url, preview: url })),
    ...galleryUploads.map((u) => ({ file: u.file, preview: u.preview })),
  ];
    ...
    >
      ...
      <FormSection title="Product Images" description="Upload product images" icon={ImageIcon}>
        <ImageUpload
          ...
          existingImage={existingPrimaryImage}
